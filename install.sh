...
echo
echo "  ${DIM}Для всех вопросов ниже можно нажать Enter, чтобы принять значение в [скобках].${RST}"
echo
[[ -z "$PUBLIC_IP" ]] && PUBLIC_IP=$(ask "Публичный IP сервера (например, 1.2.3.4)")
PORT=$(ask "UDP-порт для VPN: число от 1 до 65535" "443")
echo

# Валидация порта
if ! [[ "$PORT" =~ ^[0-9]+$ ]] || (( PORT < 1 || PORT > 65535 )); then
  err "Невалидный порт: '$PORT'"
  err "Нужно число от 1 до 65535. Например: 443, 51820, 8443"
  err "Запусти скрипт ещё раз и на этот вопрос либо нажми Enter (будет 443), либо введи число."
  exit 1
fi

PEER_NAME=$(ask "Имя первого устройства (только буквы, цифры, дефис; например my-phone)" "my-phone")
echo

# Валидация имени
if ! [[ "$PEER_NAME" =~ ^[a-zA-Z0-9._-]+$ ]]; then
  err "Невалидное имя устройства: '$PEER_NAME'"
  err "Можно только буквы, цифры, точки, подчёркивания и дефисы. Без пробелов и слешей."
  err "Запусти скрипт ещё раз и введи что-то вроде: my-phone, sergo-iphone, laptop1"
  exit 1
fi
...
echo
echo "  ${YEL}Важно:${RST} на телефоне/компьютере используй приложение ${CYA}AmneziaWG${RST}"
echo "  (именно WG в конце, не AmneziaVPN-комбайн и не обычный WireGuard)."
