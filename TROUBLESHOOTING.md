...
Симптом: в приложении AmneziaWG показывает «Подключено», но сайты не открываются.
...
## Приложение не импортит .conf

- Убедись что ставишь **AmneziaWG** (с WG в конце), а не обычный «WireGuard» (он с похожим логотипом, легко спутать). AmneziaVPN-комбайн тоже умеет, но протокол при импорте нужно выбирать вручную.
- Файл должен оканчиваться на `.conf`, не `.txt`.
- На iOS бывает помогает: отправить файл себе в Telegram → открыть из Telegram → «Поделиться» → AmneziaWG.
